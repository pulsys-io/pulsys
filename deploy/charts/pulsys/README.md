...
# Console image is not on GHCR until a release ships it — build + load.
...
Local Kind: use [`examples/cnpg-cluster-kind.yaml`](examples/cnpg-cluster-kind.yaml)
(1 instance) and [`examples/values-kind-local.yaml`](examples/values-kind-local.yaml)
as in the TL;DR above (dev Keycloak is enabled by default).
